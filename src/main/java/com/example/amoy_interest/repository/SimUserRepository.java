...
public interface SimUserRepository extends JpaRepository<SimUser, Integer> {
    @Query(value = "select new com.example.amoy_interest.dto.SimUserDTO(u.user_id, u.nickname,u.avatar_path) from SimUser as su left join UserFollow as uf  on (su.sim_id = uf.follow_id and uf.user_id = :my_user_id) join User as u on su.sim_id = u.user_id where su.user_id = :user_id and uf.follow_id is null",
