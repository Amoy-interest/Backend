...
//            nativeQuery = true)
    ...
    //需要优化？
    @Query(value = "SELECT * From blog b where b.user_id = ?1 or b.user_id in (SELECT follow_id from user u where user_id = ?1)",
            countQuery = "SELECT count(*) From blog b where b.user_id = ?1 or b.user_id in (SELECT follow_id from user u where user_id = ?1)",
            nativeQuery = true)
